...
Public Class FRM_EXTEND_TIME_LOCATION_MAIN1
    ...
    Private Sub load_HL()
        ...
        If Request.QueryString("staff") = "1" Then
            hl_pay.NavigateUrl = "https://platba.FDA.MOPH.GO.TH/FDA_FEE/MAIN/check_token.aspx?Token=" & _CLS.TOKEN & "&system=staffherb&identify=" & _identify
    ...
    Sub load_GV_lcnno()                             ' Gridview นำมาโชว์
        ...
        Dim dt As DataTable = bao.SP_LCN_EXTEND_REQUEST_BY_FK_IDA3(Request.QueryString("lcn_ida"), _process)
...
#Region "GRIDVIEW"
    ...
    Protected Sub GV_lcnno_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GV_lcnno.RowCommand
        ...
        If e.CommandName = "sel" Then
            ...
            Else
                System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & "POPUP_EXTEND_TIME_LOCATION_CONFIRM.aspx?IDA=" & dao.fields.IDA & "&TR_ID=" & tr_id & " & lct_ida = " & _lct_ida & " &lcn_ida=" & _lcn_ida & "&process=" & _process & "&staff=" & Request.QueryString("staff") & "');", True)
            End If
            'System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "Popups2('" & "POPUP_EXTEND_TIME_LOCATION_CONFIRM.aspx?IDA=" & dao.fields.IDA & "&TR_ID=" & tr_id & " & lct_ida = " & _lct_ida & " &lcn_ida=" & _lcn_ida & "&process=" & _process & "');", True)
        ...
        ElseIf e.CommandName = "_pay" Then
            ...
            If _staff = 1 Then
                Response.Write("<script>window.open ('https://platba.FDA.MOPH.GO.TH/FDA_FEE/MAIN/check_token.aspx?Token=" & _CLS.TOKEN & "&system=staffherb&identify=" & _identify & "','_blank');</script>")
            Else
