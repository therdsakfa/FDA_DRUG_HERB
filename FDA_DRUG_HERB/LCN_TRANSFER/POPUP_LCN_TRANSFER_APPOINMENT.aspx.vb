Public Class POPUP_LCN_TRANSFER_APPOINMENT
    ...
    Private Sub bind_pdf()
        ...
        Try
            date_pay = CDate(dao.fields.DATE_PAY)
        Catch ex As Exception
            date_pay = CDate(dao.fields.DATE_CONFIRM)
        End Try
        Dim formattedDate As String = date_pay.ToString("dd/MM/yyyy")
        Try
            ws.GETDATE_WORKING(CDate(formattedDate), True, days_start, True, date_result_start, True)
            ws.GETDATE_WORKING(CDate(formattedDate), True, days_end, True, date_result_end, True)
        Catch ex As Exception
            Try
        ...
        If dao.fields.STATUS_ID = 13 Then
            ...
            dao_pdftemplate.GETDATA_TABEAN_HERB_APPOINTMENT(_ProcessID, dao.fields.STATUS_ID, "APPOINMENT_LCN", 0)
            date_con_full = number_day_start
            date_appo_full = number_day_end
        Else
            dao_pdftemplate.GETDATA_TABEAN_HERB_APPOINTMENT(_ProcessID, 13, "APPOINMENT_LCN", 0)
            date_con_full = number_day_start
            date_appo_full = number_day_end
        End If
