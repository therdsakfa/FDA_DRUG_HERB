...
Public Class UC_DS_PORYORBOR8_
    ...
    Sub RunSession()
        ...
        If Request.QueryString("tt") <> "" Then
            cls_session.PVCODE = "11"
        Else
            cls_session.PVCODE = "8"
        End If
            ...
            Try
                If Len(lcnno_auto) > 0 Then
                    ...
                    If Right(Left(lcnno_auto, 3), 1) = "5" Then
                        lbl_lcnno.Text = "จ. " & CStr(CInt(Right(lcnno_auto, 4))) & "/25" & Left(lcnno_auto, 2)
                    Else
                        lbl_lcnno.Text = dao_lcn.fields.pvnabbr & " " & CStr(CInt(Right(lcnno_auto, 5))) & "/25" & Left(lcnno_auto, 2)
                    End If
    ...
    '        'If check.fields.lcntpcd = "ผยบ" Then '"ผยบ8"
    '        '    rdb_sample_drug.Checked = True
    '        '    rdb_manufacture.Checked = True
    '        '    rdb_samples_drug.Checked = True
    '        '    rdb_drug_produce.Checked = True
    '        'ElseIf check.fields.lcntpcd = "ผยบ" Then '"ผยบ8"
    '        '    rdb_direct_register.Checked = False
    '        '    rdb_direct_license.Checked = False
    '        '    rdb_direct_registers.Checked = False
    '        '    rdb_direct.Checked = False
    '        'Else
    '        '    System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "alert('เลขที่นี้ไม่มี ผยบ8 ในระบบ');", True)
    '        'End If
    '        rdb_sample_drug.Checked = True
    '        rdb_manufacture.Checked = True
    '        rdb_samples_drug.Checked = True
    ...
    Protected Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If ddl_package_unit.SelectedValue = "0" Then
            System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "alert('กรุณาเลือกขนาดบรรจุ');", True)
        ElseIf txt_qty.Text = "" Then 'Or txt_qty.Text = "0" Then
            System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "alert('กรุณากรอกปริมาณที่จะผลิต/นำสั่งให้ถูกต้อง');", True)
        Else
            ...
            Else
                Try
                    ...
                    Dim sum As Integer = CInt(dao_package.fields.SMALL_AMOUNT) * CInt(dao_package.fields.MEDIUM_AMOUNT) * CInt(dao_package.fields.BIG_AMOUNT)
                    sum = sum * CInt(txt_qty.Text)
