...
Public Class UC_DS_NORYOR8
    Inherits System.Web.UI.UserControl
    Private _CLS As CLS_SESSION
    Public _process As String = "1702"
    Private _lct_ida As Integer
    Private lcnno As String
    Private _lcn_ida As String
    Private _lcnno_display As Integer
    ...
    Public Sub set_label() 'ดึงข้อมูลแสดง
        ...
        Try
            ...
            lbl_lcnsnm.Text = dao_prefix.fields.thanm & dao_lcn.fields.syslcnsnm_thanm & " " & dao_lcn.fields.syslcnsnm_thalnm '& " " & dao_prefix.fields.thanm 'ชื่อผู้รับอนุญาต
        Catch ex As Exception
        ...
        'Dim dao_syssuffix As New DAO_CPN.TB_syssuffix
        'dao_syssuffix.Getdata_byid(dao_prefix.fields.thanm)
        'Try
        '    Dim dao_lo_bsn As New DAO_CPN.TB_LOCATION_BSN
        '    dao_lo_bsn.Getdata_by_bsnid(dao_lcn.fields.bsnid)
    ...
    End Sub
    Private Sub RadGrid1_NeedDataSource(lcnno As Integer) 'ตารางตัวยาสำคัญ
        Dim dt As New DataTable
        Dim bao As New BAO.ClsDBSqlcommand
        Try
    ...
    Private Sub RadGrid2_ItemCommand(sender As Object, e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGrid2.ItemCommand
        If TypeOf e.Item Is GridDataItem Then
            ...
            If e.CommandName = "del" Then
                Dim dao_drungname As New DAO_DRUG.TB_DRUG_REGISTRATION_PACKAGE_DETAIL
                ...
                Unit_Radgrid(dao_drungname.fields.IDA)
            End If
            RadGrid2.Rebind()
        End If

    End Sub
    Public Sub Unit_Radgrid(ByVal ida As Integer) 'ตารางขนาดบรรจุ
    ...
    Protected Sub btn_add_Click(sender As Object, e As EventArgs) Handles btn_add.Click 'ปุ่มเพิ่มขนาดบรรจุ
        ...
        Else
            ...
            Dim dao_drugname As New DAO_DRUG.ClsDBDRUG_REGISTRATION 'ตารางบัญชีรายการยา
            dao_drugname.GetDataby_IDA(main_ida) 'ปุ่มเพิ่มขนาดบรรจุ
            Try
            ...
            Catch ex As Exception
                dao.fields.MEDIUM_AMOUNT = 1        'จำนวนขนาดบรรจุกลาง
            End Try
    ...
    Protected Sub btn_save_Click(sender As Object, e As EventArgs) Handles btn_save.Click 'ปุ่มบันทึกข้อมูล
        ...
        Else
            ...
            save.fields.lcntpcd = "นย8" 'เก็บประเภทของใบอนุญาต
    ...
    Sub xml(ByVal drsamp_ida As Integer)
        ...
        cls_xml.DT_SHOW.DT3 = bao.SP_DRUG_REGISTRATION_DETAIL_CAS_FK_IDA(dao.fields.IDA) 'ตัวยาสำคัญ0
    ...
    Protected Sub btn_package_Click(sender As Object, e As EventArgs) Handles btn_package.Click
        'Dim ws As New AUTHENTICATION_104.Authentication
        'ws.AUTHEN_LOG_DATA(_CLS.TOKEN, _CLS.CITIZEN_ID, _CLS.SYSTEM_ID, _CLS.GROUPS, _CLS.ID_MENU, "DRUG", 0, HttpContext.Current.Request.Url.AbsoluteUri, "เพิ่ม/ลบ ขนาดบรรจุ", _process)
        Dim ws_118 As New WS_AUTHENTICATION.Authentication
        Dim ws_66 As New Authentication_66.Authentication
        Dim ws_104 As New AUTHENTICATION_104.Authentication
        Try
            ws_118.Timeout = 10000
            ws_118.AUTHEN_LOG_DATA(_CLS.TOKEN, _CLS.CITIZEN_ID, _CLS.SYSTEM_ID, _CLS.GROUPS, _CLS.ID_MENU, "DRUG", 0, HttpContext.Current.Request.Url.AbsoluteUri, "เพิ่ม/ลบ ขนาดบรรจุ", _process)
        Catch ex As Exception
            Try
                ws_66.Timeout = 10000
                ws_66.AUTHEN_LOG_DATA(_CLS.TOKEN, _CLS.CITIZEN_ID, _CLS.SYSTEM_ID, _CLS.GROUPS, _CLS.ID_MENU, "DRUG", 0, HttpContext.Current.Request.Url.AbsoluteUri, "เพิ่ม/ลบ ขนาดบรรจุ", _process)

            Catch ex2 As Exception
                Try
                    ws_104.Timeout = 10000
                    ws_104.AUTHEN_LOG_DATA(_CLS.TOKEN, _CLS.CITIZEN_ID, _CLS.SYSTEM_ID, _CLS.GROUPS, _CLS.ID_MENU, "DRUG", 0, HttpContext.Current.Request.Url.AbsoluteUri, "เพิ่ม/ลบ ขนาดบรรจุ", _process)
    ...
    '    Response.ContentType = "Application/pdf"
    '    Response.AddHeader("Content-Disposition", "attachment; filename=" & _CLS.PDFNAME)
    '    Response.BinaryWrite(clsds.UpLoadImageByte(_CLS.FILENAME_PDF)) '"C:\path\PDF_XML_CLASS\"0
